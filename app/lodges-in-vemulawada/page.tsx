import type { Metadata } from "next";
import Navbar from "../../components/Navbar";
import Footer from "../../components/Footer";
import { FaPhoneAlt as FaPhone, FaWhatsapp } from "react-icons/fa";

export const metadata: Metadata = {
  title: "Lodge in Vemulawada | Best Lodges & Accommodation – Moturi Farmstay",
  description:
    "Find the best lodge in Vemulawada at Moturi Farmstay. Affordable lodges near Raja Rajeshwara Temple with family-friendly accommodation in Rajanna Sircilla district, Telangana.",
  keywords: [
    "lodge in vemulawada", "lodges in vemulawada", "best lodge vemulawada",
    "cheap lodge vemulawada", "vemulawada lodge", "lodge near temple vemulawada",
    "lodge in sircilla", "sircilla lodge", "rajanna sircilla lodge",
    "accommodation vemulawada", "lodge near raja rajeshwara temple",
  ],
  alternates: { canonical: "/lodges-in-vemulawada" },
};

export default function Page() {
  return (
    <>
      <Navbar />
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-12 sm:py-16">
        <h1 className="text-3xl sm:text-5xl font-bold mb-6 text-teal-800">
          Best Lodge in Vemulawada – Moturi Farmstay
        </h1>

        <p className="text-base sm:text-lg leading-8 text-stone-700 mb-6">
          Searching for a <strong>lodge in Vemulawada</strong>? Moturi Farmstay offers the best alternative to traditional <strong>lodges in Vemulawada</strong> with a spacious farmhouse stay that gives you more space, comfort, and nature than any regular lodge. Located on Marupaka Road near Reddy Bhavan in the <strong>Rajanna Sircilla</strong> district.
        </p>

        <h2 className="text-2xl sm:text-3xl font-bold mb-4 text-teal-700">
          Better Than a Regular Lodge
        </h2>
        <ul className="list-disc pl-6 space-y-3 text-stone-700 text-base sm:text-lg mb-8">
          <li><strong>More space</strong> than a typical lodge — full 1BHK with bedroom, living room, kitchen</li>
          <li>Accommodates <strong>10 adults</strong> — perfect for groups and families</li>
          <li>Surrounded by <strong>nature, greenery, and cattle farm</strong> instead of city noise</li>
          <li><strong>Affordable pricing</strong> — better value than most lodges in Vemulawada</li>
          <li>Just 6 km from <strong>Sri Raja Rajeshwara Swamy Temple</strong></li>
          <li>Ideal for <strong>Bonalu celebrations</strong>, family trips, and friends gatherings</li>
        </ul>

        <h2 className="text-2xl sm:text-3xl font-bold mb-4 text-teal-700">
          Lodge vs Farmstay: Why Guests Prefer Us
        </h2>
        <p className="text-base sm:text-lg leading-8 text-stone-700 mb-4">
          Traditional <strong>lodges in Vemulawada</strong> typically offer small, basic rooms in the busy town centre. While they serve the purpose of overnight stays, they lack the space and environment that families and groups need. At Moturi Farmstay, you get an entire 1BHK property to yourselves — no sharing walls with strangers, no noisy corridors, and no cramped rooms.
        </p>
        <p className="text-base sm:text-lg leading-8 text-stone-700 mb-6">
          Our guests often tell us they booked looking for a <strong>lodge in Vemulawada</strong> but found something far better. The combination of private space, kitchen facilities, natural surroundings, and affordable pricing makes our farmstay the preferred <strong>accommodation in Vemulawada</strong> for repeat visitors.
        </p>

        <h2 className="text-2xl sm:text-3xl font-bold mb-4 text-teal-700">
          What You Get vs a Typical Lodge
        </h2>
        <div className="overflow-x-auto mb-8">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-teal-700 text-white">
                <th className="p-3 rounded-tl-lg">Feature</th>
                <th className="p-3">Town Lodge</th>
                <th className="p-3 rounded-tr-lg">Moturi Farmstay</th>
              </tr>
            </thead>
            <tbody className="text-stone-700">
              <tr className="bg-stone-50">
                <td className="p-3 font-medium">Room Size</td>
                <td className="p-3">Single room</td>
                <td className="p-3 text-teal-700 font-medium">Full 1BHK</td>
              </tr>
              <tr>
                <td className="p-3 font-medium">Capacity</td>
                <td className="p-3">2-3 people</td>
                <td className="p-3 text-teal-700 font-medium">Up to 10 adults</td>
              </tr>
              <tr className="bg-stone-50">
                <td className="p-3 font-medium">Kitchen</td>
                <td className="p-3">Not available</td>
                <td className="p-3 text-teal-700 font-medium">Full kitchen</td>
              </tr>
              <tr>
                <td className="p-3 font-medium">Surroundings</td>
                <td className="p-3">Town centre</td>
                <td className="p-3 text-teal-700 font-medium">Nature & farmland</td>
              </tr>
              <tr className="bg-stone-50">
                <td className="p-3 font-medium">Parking</td>
                <td className="p-3">Limited</td>
                <td className="p-3 text-teal-700 font-medium">Free & spacious</td>
              </tr>
              <tr>
                <td className="p-3 font-medium">Privacy</td>
                <td className="p-3">Shared building</td>
                <td className="p-3 text-teal-700 font-medium">Private property</td>
              </tr>
            </tbody>
          </table>
        </div>

        <h2 className="text-2xl sm:text-3xl font-bold mb-4 text-teal-700">
          Lodge Near Raja Rajeshwara Temple
        </h2>
        <p className="text-base sm:text-lg leading-8 text-stone-700 mb-6">
          Most visitors to Vemulawada look for a <strong>lodge near Raja Rajeshwara Temple</strong> for their temple visit. Moturi Farmstay offers the perfect stay — close enough to the temple for easy darshan, yet set in a peaceful natural environment away from the town crowd. Unlike regular <strong>lodges in Vemulawada</strong>, you get a full farmhouse experience with open spaces and fresh air.
        </p>

        <h2 className="text-2xl sm:text-3xl font-bold mb-4 text-teal-700">
          Lodge in Sircilla District
        </h2>
        <p className="text-base sm:text-lg leading-8 text-stone-700 mb-6">
          As a top <strong>lodge in Sircilla</strong> district, Moturi Farmstay is easily accessible from Sircilla town, Karimnagar, and Hyderabad. Whether you are looking for a <strong>lodge in Vemulawada</strong> or a <strong>lodge near Sircilla</strong>, our farmstay delivers the best value with spacious rooms, nature views, and temple proximity.
        </p>

        <h2 className="text-2xl sm:text-3xl font-bold mb-4 text-teal-700">
          Frequently Asked Questions
        </h2>
        <div className="space-y-4 mb-8">
          <div className="bg-stone-50 p-5 rounded-xl">
            <h3 className="font-bold text-teal-800 mb-2">Is this cheaper than a lodge in town?</h3>
            <p className="text-stone-600">Yes, when you consider the space you get (full 1BHK for up to 10 people), our farmstay is significantly better value than booking multiple lodge rooms.</p>
          </div>
          <div className="bg-stone-50 p-5 rounded-xl">
            <h3 className="font-bold text-teal-800 mb-2">Can we get food nearby?</h3>
            <p className="text-stone-600">Restaurants and dhabas are available in Vemulawada town (6 km). You can also cook in our kitchen. Many guests bring groceries and prepare their own meals.</p>
          </div>
          <div className="bg-stone-50 p-5 rounded-xl">
            <h3 className="font-bold text-teal-800 mb-2">Is it suitable for elderly visitors?</h3>
            <p className="text-stone-600">Yes, our property is on flat ground with easy access. The peaceful environment and fresh air are especially beneficial for elderly guests visiting the temple.</p>
          </div>
        </div>

        <div className="flex flex-wrap gap-4">
          <a href="tel:+919505834657" className="bg-green-600 hover:bg-green-700 text-white px-6 py-3 rounded-lg font-semibold transition"><FaPhone className="mr-2 inline align-[-0.125em]" />Call Now</a>
          <a href="https://wa.me/919505834657" target="_blank" rel="noopener noreferrer" className="bg-green-500 hover:bg-green-600 text-white px-6 py-3 rounded-lg font-semibold transition"><FaWhatsapp className="mr-2 inline align-[-0.125em]" />WhatsApp</a>
          <a href="/" className="bg-teal-700 hover:bg-teal-800 text-white px-6 py-3 rounded-lg font-semibold transition">🏠 Home</a>
        </div>
      </main>
      <Footer />
    </>
  );
}
